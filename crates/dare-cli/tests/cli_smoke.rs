...
fn claude_md_canonical() -> Vec<u8> {
    "<!-- dare:managed claude-md -->\n# DARE Framework\n\n\
         Generated by `dare harness claude`. Follow Design â†’ Blueprint â†’ Tasks â†’ Execute.\n\
...
fn design_preserve_notes() {
    ...
    let content = std::fs::read_to_string(dir.path().join("DARE/DESIGN.md")).expect("read");
    assert!(
        content.contains("User note outside any AGENT marker â€” must survive merge."),
...
fn execute_next_empty() {
    ...
    Command::new(cargo_bin("dare"))
        ...
        .assert()
        .success()
        .stdout(predicate::str::contains("Empty DAG â€” no tasks."));
}
