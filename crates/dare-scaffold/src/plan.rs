//! Scaffold planning (BLUEPRINT-046 §0.5 / mp046-004).
...
use crate::types::{
    PlanAction, PlanItemKind, ScaffoldPlan, ScaffoldPlanItem, ScaffoldRequest, SCHEMA_VERSION,
};

pub const PROJECT_NAME_RE: &str = r"^[a-z][a-z0-9_-]{0,63}$";
const MSG_INVALID_NAME: &str = "invalid project name";
const MSG_FRONTEND_RESERVED: &str = "frontend composition reserved for 047";
...
pub fn plan_scaffold(root: &ProjectRoot, req: &ScaffoldRequest) -> CoreResult<ScaffoldPlan> {
    ...
    if req.frontend.is_some() {
        return Err(CoreError::InvalidInput(MSG_FRONTEND_RESERVED.to_string()));
    }
    ...
    for (path, kind) in paths {
        let action = if path_exists(root, &path)? {
            if !req.force {
...
mod tests {
    ...
    fn plan_rejects_frontend() {
        ...
        let err = plan_scaffold(&root, &req).unwrap_err();
        match err {
            CoreError::InvalidInput(msg) => assert_eq!(msg, MSG_FRONTEND_RESERVED),
            other => panic!("expected InvalidInput, got {other:?}"),
        }
    }
