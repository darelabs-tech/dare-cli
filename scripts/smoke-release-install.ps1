...
$sbom = @"
{"spdxVersion":"SPDX-2.3","dataLicense":"CC0-1.0","SPDXID":"SPDXRef-DOCUMENT","name":"dare-cli-$Version","documentNamespace":"https://local/spdx/$Version","creationInfo":{"created":"$((Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ'))","creators":["Tool: dare-smoke"]},"packages":[{"name":"dare","SPDXID":"SPDXRef-Package-dare","downloadLocation":"NOASSERTION","filesAnalyzed":false,"versionInfo":"$Version"}]}
"@
Set-Content -Path (Join-Path $Out 'SBOM.spdx.json') -Value $sbom -Encoding utf8
...
& (Join-Path $prefix 'bin\dare.exe') --version
